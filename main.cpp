...
long long if_embed = 1, out_dim = -1, n_samples = -1, n_threads = -1, n_negative = -1, n_neighbors = -1, n_trees = -1, n_propagation = -1,no_maps=-1;
...
//    }
	...
	LargeVis model;
//    if (if_embed)
	model.load_from_file("/root/mnist1.txt");

//    else

    printf("n_sample is %lld\n",n_samples);
	printf("n_propagation is %lld\n",n_propagation);
	printf("alpha is %f\n",alpha);
	printf("n_tree is %lld\n",n_trees);
	printf("n_negative is %lld\n",n_negative);
	printf("n_neighbors is %lld\n",n_neighbors);
	printf("n_gamma is %f\n",n_gamma);
	printf("perplexity is %f\n",perplexity);
	model.run(out_dim, n_threads, n_samples, n_propagation, alpha, n_trees, n_negative, n_neighbors, n_gamma, perplexity,no_maps);
	model.save("/root/verticls.txt","/root/weight.txt");
	printf("finish whole process");
}
